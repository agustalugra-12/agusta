...
- [x] **Bug ditemukan & diperbaiki (2026-07-12, sama hari)**: hari check-out booking menginap selalu ikut terhitung "terisi" di kalender ketersediaan (mis. checkin 20/checkout 21 tampil terisi di 20 **dan** 21, padahal cuma 1 malam — seharusnya cuma tanggal 20). Root cause: `kalender_bulanan`/`ketersediaan_hari` (`backend/routes/ketersediaan.py`) pakai overlap TIMESTAMP mentah (`b_end >= day_start`) yang secara matematis benar untuk anti-overbooking tapi salah untuk semantik "kalender per-malam" manusia. Diperbaiki dengan helper `_occupies_date` — occupancy sekarang berbasis rentang TANGGAL `[checkin_date, checkout_date)` (checkout day dikecualikan), kecuali day-use (checkin=checkout, tetap terhitung terisi hari itu). Bug ini sistemik (semua booking menginap, bukan cuma yang dibuat AI Email Parser) tapi ketahuan dari laporan user soal alur OTA. Diverifikasi 11 skenario standalone, semua lolos.
- [x] **Lanjutan bug di atas ditemukan lagi (2026-07-12, sama hari)**: setelah fix pertama di-deploy, user lapor booking Widya (checkin 12/checkout 13) MASIH tidak muncul tersedia di tanggal 13. Investigasi: kalender Ketersediaan (`ketersediaan.py`) ternyata SUDAH benar (dicek via API live dengan login owner + curl), tapi `GET /public/availability` (`routes/public.py`, dipakai pencarian kamar di form booking publik `/book`) adalah endpoint TERPISAH yang masih pakai overlap timestamp mentah yang sama — belum ikut diperbaiki batch sebelumnya. Dikonfirmasi lewat curl langsung ke API produksi: room Cottage #1 (booking Widya) hilang dari daftar tersedia tanggal 13 sebelum fix. Diperbaiki dengan `_booking_date_range` (logika sama seperti `_occupies_date`), diverifikasi 4 skenario standalone + re-test curl live setelah deploy (room muncul lagi).
- [x] **Bug ketiga dengan akar sama, kali ini di FRONTEND (2026-07-12, sama hari)**: user masih lapor "masalah room Widya masih sama" setelah 2 fix backend di atas. Ternyata `Dashboard.jsx` (halaman utama staf, grid kamar) punya filter `bookingsOnDate` client-side sendiri yang juga overlap timestamp mentah (`start <= dayEnd && end >= dayStart`) — inilah yang paling mungkin dilihat user karena Dashboard paling sering dibuka. Diperbaiki dengan logika rentang tanggal yang sama (exclusive checkout day, kecuali day-use). Diverifikasi unit test JS standalone (4 skenario termasuk kasus Widya persis), semua lolos.
...
- [x] `GET /api/payments/log` (2026-07-14) — daftar semua transaksi payment_log lintas booking (join `nama_tamu`, filter status/search), dipakai tabel utama `Pembayaran.jsx` (sekarang bukan mock lagi).
...
- [x] Dialog "Buat Tagihan Baru" disambungkan ke Tripay sungguhan (2026-07-14) — lihat detail di section "Integrasi Pembayaran (Tripay, sebelumnya Midtrans)" di bawah.
...
- [x] **Bug fatal ditemukan & diperbaiki (2026-07-12, sama hari)**: `OTA_QUERY` dari awal proyek cuma mencari domain agoda.com/traveloka.com/booking.com — **tidak pernah mencari reddoorz.com**, jadi auto-fetch di atas berjalan tapi tidak pernah menemukan email RedDoorz nyata milik user. Sesuai instruksi user: PMS sekarang **hanya fokus** ke `bookings-indonesia@reddoorz.com` di `agus.lugra@gmail.com` (OTA lain dilepas dari scope). Setelah fix, 19 email lama yang sempat nyangkut diproses ulang manual (2 pemetaan tipe kamar baru dibuat, 18 reservasi nyata masuk PMS, 1 dikoreksi jadi cancelled karena urutan modifikasi vs baru). Bug turunan ikut diperbaiki: `batalkan_reservasi_otomatis` tidak mengembalikan status log ke `Parsed_Success` saat sukses.
- [x] **Revisi 1 — modifikasi OTA jadi perlu review staf, bukan auto-cancel (2026-07-12, keputusan bisnis user, sama hari)**: notifikasi "modifikasi" OTA dianggap ambigu (bisa reschedule ATAU pembatalan terselubung) — bukan lagi auto-cancel disamakan dengan pembatalan eksplisit. Booking terkait ditandai `modifikasi_status="menunggu_review"`, muncul di tab baru "Modifikasi OTA" (halaman Otomasi Email) untuk staf konfirmasi lewat 2 endpoint baru: `POST /otomasi-email/modifikasi/{id}/reschedule` (update jadwal, harga tidak berubah) dan `POST /otomasi-email/modifikasi/{id}/batalkan`. Email modifikasi susulan untuk reservasi yang sama tidak menumpuk item review ganda (dicatat sebagai duplikat).
- [x] **Revisi 2 — auto-tentukan reschedule vs pembatalan dari tanggal email (2026-07-12, keputusan bisnis user, sama hari, menggantikan sebagian Revisi 1)**: AI Email Parser sekarang juga coba ekstrak check-in/check-out BARU dari isi email modifikasi (kalau disebutkan eksplisit, tidak dikarang). `proses_modifikasi_otomatis` membandingkan dengan jadwal lama di PMS: **check-in DAN check-out baru sama persis** dengan yang lama → dianggap pembatalan terselubung, auto-cancel (lewat `batalkan_reservasi_otomatis`); **salah satu berbeda** (mis. check-in tgl 6 jadi tgl 9, atau cuma checkout yang berubah/diperpanjang) → auto-reschedule ke jadwal baru (tetap dicek anti-overbooking). Tab "Modifikasi OTA" (dari Revisi 1) sekarang jadi **fallback**, cuma dipakai kalau AI tidak berhasil menemukan tanggal baru yang jelas, atau jadwal baru bentrok kamar lain — staf yang putuskan manual di kasus itu. Diverifikasi lewat skrip standalone (6 skenario: tanggal sama, checkin beda, checkout beda saja, data kosong, bentrok kamar, email susulan duplikat) — semua lolos, data uji dibersihkan.
- [x] **Bug ditemukan & diperbaiki (2026-07-13)**: dedup guard di `proses_modifikasi_otomatis` (`existing_status in ("menunggu_review", "direschedule", "dibatalkan")`) memperlakukan SEMUA email modifikasi susulan sebagai duplikat murni begitu status booking sudah `direschedule` — tidak pernah sampai ke pengecekan "tanggal sama = batal". Kasus nyata yang dilaporkan user: reservasi no. OTA 444263135238733 di-reschedule otomatis ke 2026-07-20/21, lalu RedDoorz kirim email modifikasi kedua dengan tanggal SAMA PERSIS dengan hasil reschedule itu (seharusnya berarti pembatalan terselubung), tapi dedup guard keburu menghentikan proses dan menandainya "Sudah_Diproses" tanpa aksi apa-apa — booking tetap `booking_pending`, kamar tetap dianggap terisi/menginap. Diperbaiki: dedup guard sekarang hanya berlaku untuk status `menunggu_review`; status `direschedule` tetap diproses lagi, dibandingkan terhadap jadwal booking yang SEKARANG tersimpan (hasil reschedule terakhir), bukan jadwal asli — `backend/routes/otomasi_email.py`. Booking yang sudah terlanjur tersangkut (BKO-20260712201928-83B1) dikoreksi manual (dibatalkan lewat `batalkan_reservasi_otomatis`, kamar dilepas) setelah izin user.
...
- [x] **Buat Tagihan Baru — sekarang nyata (2026-07-14)**: dialog di `Pembayaran.jsx` sekarang memanggil `POST /payments/tripay/create-transaction` sungguhan (bukan simulasi lagi) — daftar booking diambil dari `GET /payments/bookings-status?status_bayar=belum_bayar` (nyata), channel dari `GET /payments/tripay/channels` (nyata, live dari Tripay). Backend `tripay_create_transaction` (`backend/routes/tripay.py`) diperluas: sebelumnya cuma terima booking status `booking_pending` (khusus alur publik tamu), sekarang juga terima status `aktif` (booking staf lewat Quick Book untuk reservasi telepon/WA) — asal `payment_status` belum `paid`. Kredensial masih sandbox (`TRIPAY_IS_PRODUCTION=false`); begitu Tripay approve kredensial produksi, tinggal ganti env var `TRIPAY_*` di server + restart service, tidak ada perubahan kode. Diverifikasi lewat skrip standalone (booking status aktif → tagihan berhasil dibuat dengan `checkout_url` Tripay sandbox asli; booking sudah lunas → ditolak; booking cancelled → ditolak), data uji dihapus.
...
- [x] **Retry bayar untuk booking `cancelled` (expired/gagal bayar) — SEKARANG NYATA (2026-07-14)**: endpoint baru `POST /public/bookings/{id}/retry-bayar` (`backend/routes/public.py`) membuka lagi booking yang dibatalkan OTOMATIS oleh webhook payment gateway (dibedakan dari pembatalan mandiri tamu/staf/OTA lewat kombinasi `payment_status in (expired, failed)` + tidak ada `cancelled_by` — webhook gateway tidak pernah mengisi field itu), re-cek ketersediaan kamar (`check_room_available`) supaya tidak double-booking kalau slotnya sudah diambil tamu lain. Frontend: tombol "Coba Bayar Lagi" di `/book/sukses` (`PublicBook.jsx`, `RetryBayarDialog`) — pilih channel Tripay + opsi DP50/Lunas, lalu panggil retry-bayar + `create-transaction` berurutan dalam satu submit (supaya tidak ada booking menggantung status `booking_pending` tanpa transaksi aktif), redirect ke `checkout_url` Tripay. Diverifikasi lewat skrip standalone (5 skenario: expired → sukses; dibatalkan mandiri tamu → ditolak; dibatalkan staf → ditolak; kamar sudah dipakai booking lain → ditolak; status bukan cancelled → ditolak), data uji dihapus.
...
- [x] **Voucher PDF tampilkan DP & sisa bayar (2026-07-12, permintaan user)**: `generate_voucher_pdf` (`backend/email_service.py`) tambah baris "Sudah Dibayar (DP)" dan "Sisa Bayar", dihitung dari `total` vs `amount_due` (field yang sama dipakai fitur Status Bayar staf). Berlaku di voucher unduh tamu (`GET /public/bookings/{id}/voucher.pdf`) maupun kirim otomatis via email.
- [x] **Panel transfer manual dihapus dari halaman sukses booking (2026-07-12, permintaan user)**: "Alternatif: Transfer Manual ke Rekening" di `PublicBook.jsx` `SuccessView` dihapus total (state/fetch `banks` ikut dibersihkan). Endpoint `GET /public/bank-accounts` tetap ada (masih dipakai test lama), cuma tidak dipanggil dari halaman ini lagi.
...
- [x] **Bug ditemukan & diperbaiki (2026-07-12, sama hari)**: user melaporkan `https://pmspelangi.my.id/book/sukses/` (tanpa ID booking di akhir URL) tidak menampilkan halaman sukses/voucher — route React (`App.js`) cuma mendaftarkan `/book/sukses/:bookingId` (wajib ada ID), jadi URL tanpa ID jatuh ke catch-all `*` dan redirect ke `/` yang ter-proteksi, ujungnya nyasar ke halaman **login admin** — sangat membingungkan untuk tamu publik. Kemungkinan penyebab nyata: Midtrans Dashboard punya "Finish Redirect URL" *statis per akun* untuk sebagian metode pembayaran (tidak semua channel mendukung override per-transaksi lewat `callbacks.finish` di API), jadi kalau field itu diisi tanpa placeholder ID, browser tamu akan mendarat di URL persis tanpa ID setelah bayar. Diperbaiki: (1) `App.js` tambah route `/book/sukses` (tanpa ID) yang tetap merender `SuccessView`; (2) `PublicBook.jsx` sekarang menyimpan `bk.id` ke `localStorage` (`pelangi_last_booking_id`) begitu booking dibuat (sebelum Snap dibuka); (3) `SuccessView` fallback ke ID di localStorage kalau URL tidak menyertakan `:bookingId`, lalu membetulkan URL (`navigate(..., {replace:true})`) supaya halaman tetap bisa di-refresh/bookmark; (4) kalau ID tetap tidak ketemu sama sekali (device beda / localStorage kosong), tampilkan kartu "Detail Booking Tidak Ditemukan" dengan tombol WhatsApp CS & link booking baru — bukan lagi nyasar ke login admin. Diverifikasi dengan Playwright (headless Chromium) atas build produksi + booking uji nyata di API live (3 skenario: URL dengan ID, URL tanpa ID tanpa localStorage, URL tanpa ID dengan localStorage) — semua lolos, booking uji dihapus dari DB setelahnya.
